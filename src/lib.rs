    ...
    pub async fn run(
        ...
        HttpServer::new(move || {
            App::new()
                ...
                .service(
                    web::resource("/add_device").route(web::post().to(handlers::add_device::<T>)),
                )
                ...
                .service(
                    web::resource("/devices/{room}").route(web::get().to(handlers::devices::<T>)),
                )
                .service(
                    web::resource("/{room}").route(web::delete().to(handlers::remove_room::<T>)),
                )
                .service(
                    web::resource("/{room}/{device}")
                        .route(web::delete().to(handlers::remove_device::<T>)),
                )
        })
        .bind("127.0.0.1:8080")?
        .run()
